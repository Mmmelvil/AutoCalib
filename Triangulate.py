...
def triangulateLinearEigen(cameras, x):
    ...
    P = np.asarray([cam.P for cam in cameras]) # [N,3,4]
    print(P)
...
def baseFuncMVMP(cameras, x):
    ...
    b = np.asarray(
        [cameras[n].get3DLine(x[n, :2, 0] / x[n, 2, 0], format='TdL') for n in range(len(cameras))])  # [N,2,3]
...
def triangulateIRMP(cameras, x, eps=1e-15, maxIters=500):
    ...
    while (not lastStep or lastStep >= eps) and i < maxIters:
        ...
        e = (np.linalg.norm(B @ (X[None, :, :] - o[:, :, None]), ord=2, axis=1)[:, 0] ** 2) / (np.linalg.norm(
            X[None, :, :] - o[:, :, None], ord=2, axis=1) ** 2)[:, 0]  # [N]
...
def baseFuncLM(cameras, x, XInit, eps1=1e-7, eps2=1e-15, maxIters=500, tau=1e-4, v=2):
    ...
    f = (x[:, :2, :] / x[:, 2, None, :] - (P[:, :2, :] @ X[None, :, :]) / (P[:, 2, None, :] @ X[None, :, :])).reshape(
        (2 * x.shape[0], 1))  # Current loss vector, [2N,1]. [4,1]->[2N,1]
    F = (.5 * f.T @ f)[0, 0]  # [1,]
    L = F  # [1,]
    J = ((P[:, :2, :] * (P[:, 2, None, :] @ X[None, :, :]) - P[:, 2, None, :] * (P[:, :2, :] @ X[None, :, :])) / (
                (P[:, 2, None, :] @ X[None, :, :]) ** 2)).reshape((2 * x.shape[0], 4))  # Current Jacobian of f, [2N,4]
    ...
    while not found and (np.linalg.norm(g, ord=np.inf) >= eps1) and i < maxIters:
        ...
        fNext = (x[:, :2, :] / x[:, 2, None, :] - (P[:, :2, :] @ (X + nextStep)[None, :, :]) / (
                    P[:, 2, None, :] @ (X + nextStep)[None, :, :])).reshape((2 * x.shape[0], 1))
        FNext = (.5 * fNext.T @ fNext)[0, 0]  # [1,]
        LNext = FNext + (nextStep.T @ J.T @ f)[0, 0] + .5 * (nextStep.T @ J.T @ J @ nextStep)[0, 0]  # [1,]
        JNext = ((P[:, :2, :] * (P[:, 2, None, :] @ (X + nextStep)[None, :, :]) - P[:, 2, None, :] * (
                    P[:, :2, :] @ (X + nextStep)[None, :, :])) / (
                             (P[:, 2, None, :] @ (X + nextStep)[None, :, :]) ** 2)).reshape((2 * x.shape[0], 4))
...
cam1_intr = readCameraMatrix('./Intrinsics/CornerNW.intrinsics.json')
cam2_intr = readCameraMatrix('./Intrinsics/WallS.intrinsics.json')

cam1_extr = readExtrinsics('./Extrinsics/CornerNW.extrinsics.position.json')
cam2_extr = readExtrinsics('./Extrinsics/WallS.extrinsics.position.json')


CamNW = Camera(camMat=cam1_intr['C_mat'], distortion=cam1_intr['d_coeffs'])
CamNW.setExtrinsicParamsInv(view=cam1_extr['extrMat'])

CamS = Camera(camMat=cam2_intr['C_mat'], distortion=cam2_intr['d_coeffs'])
CamS.setExtrinsicParamsInv(view=cam2_extr['extrMat'])


pts_l = np.array(json.load(open('./Extrinsics/CornerNW.extrinsics.points.json'))['imgPoints'])
pts_r = np.array(json.load(open('./Extrinsics/WallS.extrinsics.points.json'))['imgPoints'])


triangulated_points = cv2.triangulatePoints(CamNW.P, CamS.P, np.transpose(pts_l), np.transpose(pts_r))
...
cameras = [CamNW, CamS]
print(pts_r[0,:])
print(np.vstack((pts_l[0,:], pts_r[0,:])).shape)

triangulated_points_lateMVMP = np.zeros((len(pts_l), 3), dtype=float)


for point_count in range(len(pts_l)):
    triangulated_points_lateMVMP[point_count,:] = triangulateMPLM(cameras, np.vstack((pts_l[point_count,:], pts_r[point_count,:])))

print(triangulated_points_lateMVMP.shape)
